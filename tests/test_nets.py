...
from leibnetz.nets.scalenet import build_subnet as build_scalenet_subnet, build_scale_net
from leibnetz.nets.attentive_scalenet import build_subnet as build_attentive_subnet, build_attentive_scale_net
...
class TestScaleNet(unittest.TestCase):
    ...
    def test_build_scale_net_basic(self):
        """Test basic ScaleNet construction"""
        scale_net = build_scale_net([
            {"top_resolution": (8, 8, 8), "base_nc": 4}
        ])
        
        self.assertIsInstance(scale_net, LeibNet)

    def test_build_scale_net_multiple_scales(self):
        ...
        scale_net = build_scale_net(subnet_dict_list)
        
        self.assertIsInstance(scale_net, LeibNet)
        self.assertGreater(len(scale_net.output_keys), 0)

    def test_build_scale_net_forward_pass(self):
...
class TestNetsIntegration(unittest.TestCase):
    ...
    def test_nets_have_valid_input_output_keys(self):
        ...
        scale_net = build_scale_net([{"base_nc": 2}])
